...
class timm_feature_extractor(Module):    
    def __init__(self, model, output_stride=True, input_dim=None, 
        ...
        if output_stride==False: #if not, standard spatial dimms are kept
            timm.create_model(model, features_only=True, pretrained=True)
            ...
            try:
                ...
                #self.net.load_state_dict(torch.load('model_weights.pth'), strict=False)
                #print('load custom weights3')
            except:
                try:
